...
const AdministrativeExpensesList: React.FC<AdministrativeExpensesListProps> = ({ refresh }) => {
  ...
  const [filteredExpenses, setFilteredExpenses] = useState<AdministrativeExpense[]>([]); // Gastos después de aplicar filtros
  const [displayedExpenses, setDisplayedExpenses] = useState<AdministrativeExpense[]>([]); // Gastos mostrados en la página actual
  ...
  const updateDisplayedExpenses = (expenses: AdministrativeExpense[], page: number) => {
  ...
  const applyDateFilters = () => {
    ...
    const newFilteredExpenses = allExpenses.filter(expense => {
      ...
      console.log(
        ...
        "Timestamp:", expense.date.seconds,
        "¿Dentro del rango?:", result
      );
      return result;
    });
  ...
  const applyCurrentDateFilter = () => {
    ...
    const startDate = new Date(`${current.month}/${current.day}/${current.year}`);
  ...
  const resetFilters = () => {
    ...
    setFilteredExpenses(allExpenses);
    setCurrentPage(1); // Volver a la primera página
    updateDisplayedExpenses(allExpenses, 1);  };  // Función para exportar a Excel
  const exportToExcel = async () => {
    try {
      ...
      filteredExpenses.forEach((expense: AdministrativeExpense) => {
        worksheet.addRow({
          fecha: new Date(expense.date.seconds * 1000).toLocaleDateString('es-ES', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric'
          }),
  ...
  return (    <>    <div className="text-center mt-8 w-full bg-gradient-to-b from-blue-100 to-blue-200 h-8 rounded-lg shadow-lg">
      ...
      >
        {isExporting ? (
          ...
          <>
            ...
            Exportar a Excel{filteredExpenses.length > 0 ? ` (${filteredExpenses.length} registros)` : ''}
          </>
        )}
      </button>
    </div>
    ...
    <div className="bg-white p-4 mt-4 shadow-md rounded-lg">
      <h2 className="text-sm md:text-md font-semibold text-blue-800 mb-3">Filtrar por rango de fecha</h2>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <div className="w-24">
              ...
              >
                ...
                {months.map(month => (
                  <option key={`start-month-${month}`} value={month}>{month}</option>
                ))}
              </select>
            </div>
            <div className="w-24">
      ...
      <div className="bg-white p-4 md:p-8 shadow-lg rounded-lg sm:overflow-y-auto sm:max-h-96 md:overflow-visible md:max-h-full" ref={tableRef}>
      <div className="overflow-x-auto">
        <table className="table-auto w-full border-collapse border border-gray-300">
          <thead>
            <tr>
              <th className="border border-gray-300 bg-blue-100 px-4 py-2">Fecha</th>
              <th className="border border-gray-300 bg-blue-100 px-4 py-2">Tipo de Gasto</th>
          ...
          <tbody>
            {displayedExpenses.length > 0 ? (
              displayedExpenses.map((expense: AdministrativeExpense, index: number) => (
                <tr key={index}>
      ...
      {filteredExpenses.length > 0 && (
        <div className="flex flex-col md:flex-row justify-between items-center mt-6 px-2 py-3 bg-blue-50 rounded-lg">
          <div className="text-sm text-gray-600 mb-3 md:mb-0">
            Mostrando {displayedExpenses.length} de {filteredExpenses.length} registros
          </div>
          <div className="flex items-center space-x-2">            <button
              onClick={() => changePage(1)}
              disabled={currentPage === 1}
              className={`px-3 py-1.5 rounded text-sm ${currentPage === 1 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-blue-100 text-blue-800 hover:bg-blue-200'}`}
            >
              &lt;&lt;
            </button>
            <button
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded text-sm ${currentPage === totalPages ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-blue-100 text-blue-800 hover:bg-blue-200'}`}
            >
              &gt;
            </button>
            <button
            ...
            >
              ...
              className="ml-4 px-3 py-1.5 text-sm border border-blue-300 rounded bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            >
